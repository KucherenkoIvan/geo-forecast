...
async function loadTrackList() {
  try {
    ...
    trackIds = data.map((track) => track.ID);
  } catch (error) {
...
async function loadGeoData() {
  try {
    ...
    const response = await fetch(`/api/tracks?trackId=${selectedTrack}`);
    const data = await response.json();

    // Sort logs by timestamp
    const sortedLogs = data.geo_position_logs.sort(
    ...
    coordinates.forEach((coord, index) => {
      ...
      point.bindPopup(
        `Point ${index + 1}<br>Time: ${date.toLocaleTimeString()}`
      );
    });
  ...
  function animate(currentTime) {
    ...
    if (newTimestamp > timestamps[timestamps.length - 1]) {
      ...
      if (newIndex !== lastIndex) {
        ...
        if (arrowElement) {
          arrowElement.querySelector('div').style.transform =
            `rotate(${currentDirection}deg)`;
        }
        lastIndex = newIndex;
      }
